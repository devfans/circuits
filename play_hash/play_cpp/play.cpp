...
void play_100_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void play_100_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 818337;}

uint get_number_of_components() {return 7232;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 117306;}

uint get_size_of_constants() {return 336;}
...
Fr_band(&expaux[0],&expaux[1],&circuitConstants[335]); // line circom 83
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
Fr_band(&expaux[0],&expaux[1],&circuitConstants[335]); // line circom 85
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_band(&expaux[0],&expaux[1],&circuitConstants[335]); // line circom 86
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_band(&expaux[0],&expaux[1],&circuitConstants[335]); // line circom 91
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_band(&expaux[0],&expaux[1],&circuitConstants[335]); // line circom 95
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_bxor(&expaux[3],&circuitConstants[335],&lvar[0]); // line circom 31
...
Fr_band(&expaux[0],&expaux[1],&circuitConstants[335]); // line circom 7
Fr_copy(destination,&expaux[0]);
return;
}
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[151]); // line circom 20
while(Fr_isTrue(&expaux[0])){
{
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[153]); // line circom 25
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 768)];
// load src
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[137]); // line circom 29
while(Fr_isTrue(&expaux[0])){
Fr_sub(&expaux[1],&circuitConstants[154],&lvar[2]); // line circom 30
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 30
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[0])) + 768)];
// load src
Fr_shr(&expaux[1],&circuitConstants[151],&lvar[2]); // line circom 30
Fr_band(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 30
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((1 * Fr_toInt(&lvar[2])) + 512)];
// load src
Fr_mul(&expaux[1],&lvar[1],&circuitConstants[151]); // line circom 73
...
ctx->componentMemory[coffset].templateName = "play";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 512;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "sha256_1";
...
{
uint aux_create = 1;
int aux_cmp_num = 5421+ctx_index+1;
uint csoffset = mySignalStart+613816;
for (uint i = 0; i < 1; i++) {
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 512],256);
}
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[136]); // line circom 19
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 2;
{
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 19
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[136]); // line circom 19
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[136]); // line circom 22
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 2;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((1 * (256 + Fr_toInt(&lvar[0]))) + 256)];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + ((1 * Fr_toInt(&lvar[0])) + 0)]);
}
...
play_100_create(1,0,ctx,"main",0);
play_100_run(0,ctx);
}
